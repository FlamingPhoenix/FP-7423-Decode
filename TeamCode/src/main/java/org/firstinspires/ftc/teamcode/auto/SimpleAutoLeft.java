...
@Autonomous(name = "Simple Auto Left", group = "Autonomous")
@Configurable
public class SimpleAutoLeft extends OpMode {
    ...
    public static class Paths {
        ...
        public Paths(Follower follower) {
            ...
            Path5 = follower.pathBuilder().addPath(
                new BezierCurve(
                  new Pose(23.000, 86.500),
                  new Pose(23.093, 81.249),
                  new Pose(17.375, 77.718)
                )
              ).setLinearHeadingInterpolation(Math.toRadians(0), Math.toRadians(0))
    ...
    public int autonomousPathUpdate() {
        switch (pathState) {
            ...
            case 17:
                if (!follower.isBusy()) {
                    // Turn on intake after Path7 completion
                    intake.setPower(-0.9);
                    wheel.setPower(1);
                    pathTimer.resetTimer();
                    setPathState(18);
                ...
                break;
            case 18:
                // Brief pause before starting Path8
                if (pathTimer.getElapsedTimeSeconds() > 0.1) {
                    follower.followPath(paths.Path8, 0.3, true);
                    setPathState(19);
                }
                break;
            case 19:
                if (!follower.isBusy()) {
                    // Start charging up shooter after Path8 since it's farther away
            ...
            case 20:
                ...
                break;
            case 21:
                // 100ms wait after final intake
                if (pathTimer.getElapsedTimeSeconds() > 0.1) {
                    // Stop intake motor but keep wheel running for final sequence
                    intake.setPower(0);
                    // Use higher speeds for final sequence to improve consistency
                    shooterSpeed = -1375;
                    firstBallSpeed = -1250;
                    startShooting(); // Trigger normal shooting sequence
                    setPathState(22);
